...
import "/src/page/auth/style/register.sass"


export function Register(){
    const [authData, setAuthData] = useState(
        {"user_name": null, "email": null, "password": null}
    )
    ...
    return(
        <section className="auth register">
            <h2 className="hidden">
                Форма для регистраии
            </h2>
            <div className="wrapper">

            <form className="auth__form auth__form_big" onSubmit={(event) => register(event)}>
                ...
                <div className="auth-input__container">
                    <AuthInput
                        ...
                        required 
                        autoComplete="off"
                        onChange={(event) => setAuthData({...authData, "user_name": event.target.value})}
                    />
                ...
                <div className="userManual">
                    <input className="userManual-input" type="checkbox" id="userManual-input" required/>
                    <label className="userManual-checkbox" htmlFor="userManual-input">
                        Мною прочитаны и приняты
                        <Link className="auth__link" to="#"> Пользовательское соглашение </Link>
                        и
                        <Link className="auth__link" to="#"> Политика конфиденциальности </Link>
                    </label>
                </div>
